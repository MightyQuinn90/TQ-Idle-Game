import type { Player, ItemId, Item } from "../types/types.js"
...
export function openChest(player: Player, chestId: ItemId){
...
export function isInventorySlotsFull(player: Player, itemIdToAdd: ItemId): boolean{
    const hasItem = player.inventory.some(i => i.itemId === itemIdToAdd)
    if(hasItem) return false

    return player.inventory.length >= player.maxSlots
}
export function hasRoomForNewItem(player: Player, itemId: string): boolean{
